...
def _render_dataset_info(benchmark: str, frequency: str, start_date: str, end_date: str, normalization: dict | None) -> None:
    base_items = [
    ...
    if normalization:
        norm_scaling = normalization.get("scaling") or "N/A"
        ...
        trim_val = f"{normalization.get('trimPct')}%" if normalization.get("trimPct") is not None else "N/A"
        precision_val = str(normalization.get("precision")) if normalization.get("precision") is not None else "N/A"
        outlier_val = str(normalization.get("outlierLimit")) if (normalization.get("outliers") and normalization.get("outlierLimit") is not None) else "N/A"

        norm_items = [
            _info_item("Scaling", norm_scaling),
            _info_item("Scope", scope_val),
            _info_item("Trim", trim_val),
            _info_item("Precision", precision_val),
    ...
    for ds_ver in sorted_datasets:
        ...
        if dataset_info:
            ...
            with st.container(border=True):
                ...
                _render_dataset_info(benchmark, frequency, start_date, end_date, normalization)
