...
def sidebar() -> st.navigation:
    ...
    with st.sidebar:
        ...
        st.markdown(f"<h4 style='padding: 0; margin: 0;'>{fl_name} ({fl_id})</h4>", unsafe_allow_html=True)
        spacer(1)
        st.divider()
        spacer(1)
        ...
        st.page_link(about_page, label="About", icon=":material/info:", query_params={"fl_id": fl_id})
